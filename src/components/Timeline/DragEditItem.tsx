...
const DragEditItem = ({
    ...
    const recalculateEvent = () => {
        ...
        const newEvent = {
            ...
            start: currentDate,
            end: currentDate.add(eventHeight.value / heightByTimeInterval.value, 'hour'),
        };
...
const AnimatedHour = ({
    ...
    const _onChangedTime = (
        ...
        if (hourFormat) {
            newTime = new Date(1970, 1, 1, typeof hourStr === 'string' ? parseInt(hourStr) : hourStr, typeof minutesStr === 'string' ? parseInt(minutesStr) : minutesStr).format(hourFormat);
        }
        setTime(newTime);
    };
