...
export const calculateDates = (
    ...
    const minWeekDate = minDate.subtract(diffBefore, 'day');
...
export const calculateHours = (
    ...
    while (tempStart < end) {
        ...
        if (hourFormat) {
            time = new Date(1970, 1, 1, parseInt(hourStr), parseInt(minuteStr)).format(hourFormat);
        }
...
export const groupEventsByDate = (
    ...
    events.forEach((event) => {
        const startEvent = startOfDay(event.start);
...
const buildEvent = (
    ...
    const timeToHour = eventStart.getHours() + eventStart.getMinutes() / 60;
    let start = timeToHour - options.startHour;
    const diffHour = eventEnd.diff(eventStart, 'minute') / 60;
    const isSameDate = eventStart.isSame(eventEnd, 'day');
    if (!isSameDate) {
